...
import { getListingBySlug, getListerListings, getListerProfile } from "@/lib/catalog";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  // Priority 1: Listing
  const listing = await getListingBySlug(identifier);
  if (listing) {
    return {
      ...
      openGraph: {
        ...
        description: listing.tagline,
        type: "website",
        url: `https://sassymartie.com/${identifier}`,
        images: ["/brand/logo.png"],
      },
    };
  }

  // Priority 2: Lister Profile
  const profile = await getListerProfile(identifier);
  if (profile) {
    return {
...
export default async function DynamicPage({ params }: PageProps) {
  ...
  // 1. Try to render as a Listing
  const listing = await getListingBySlug(identifier);
  if (listing) {
    const moreFromBuilder = await getListerListings(listing.lister_id);
    const otherTools = moreFromBuilder.filter(t => t.slug !== identifier).slice(0, 3);
              ...
              <div className="card-peach p-8 rounded-xl border border-brand-red/10 mb-12">
                ...
                <div className="mt-6 border-t border-brand-red/10 pt-6">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {listing.trust?.active_badges && listing.trust.active_badges.length > 0 ? (
  ...
  // 2. Try to render as a Lister Profile
  const profile = await getListerProfile(identifier);
  if (profile) {
